...
writhdeck-android/
  app/src/main/
    assets/tcl/
      boot-android.tcl          <- committed — Android startup stub
      state.tcl                 <- generated by Gradle from ../writhdeck/src/state.tcl
      config.tcl                <- generated by Gradle from ../writhdeck/src/config.tcl
      schemes/*.tcl             <- generated by Gradle from ../writhdeck/src/schemes/
      lib/tcl8.6/               <- committed — Tcl stdlib (stable across 8.6.x)
    cpp/
      CMakeLists.txt
      writhdeck_jni.c           <- JNI bridge
      include/                  <- Tcl headers (tcl.h, tclDecls.h...)
    java/com/writhdeck/app/
      WrithdeckEngine.kt        <- Kotlin / JNI wrapper
      WrithdeckViewModel.kt     <- Compose ViewModel
      MainActivity.kt
      ui/
        BrowserScreen.kt
        EditorScreen.kt
  tcl-android/                  <- generated by build-tcl-android.sh (git-ignored)
    arm64-v8a/libtcl8.6.a
    x86_64/libtcl8.6.a
  tcl8.6.15/                    <- extracted Tcl sources (git-ignored)
  tools/
    build-tcl-android.sh
```

### Git status of generated files

| File / Folder | Git status | How to obtain |
|---|---|---|
| `tcl8.6.15/` | ignored | `wget` + `tar` — required to compile the `.a` |
| `tcl-android/arm64-v8a/libtcl8.6.a` | ignored | `./tools/build-tcl-android.sh arm64-v8a` |
| `tcl-android/x86_64/libtcl8.6.a` | ignored | `./tools/build-tcl-android.sh x86_64` |
| `app/src/main/assets/tcl/lib/tcl8.6/` | **committed** | Tcl stdlib — stable across 8.6.x |
| `app/src/main/assets/tcl/state.tcl` | ignored | copied by Gradle from `../writhdeck/src/state.tcl` |
| `app/src/main/assets/tcl/config.tcl` | ignored | copied by Gradle from `../writhdeck/src/config.tcl` |
| `app/src/main/assets/tcl/schemes/*.tcl` | ignored | copied by Gradle from `../writhdeck/src/schemes/` |
| `gradle/wrapper/gradle-wrapper.jar` | not committed | `gradle wrapper --gradle-version=8.9` or direct download |
| `app/src/main/res/mipmap-*/ic_launcher.png` | not committed | app icon — Android uses default icon if absent |
...
Intent-filter `ACTION_VIEW` / `ACTION_EDIT` + `text/plain` declared in `AndroidManifest.xml`.
`MainActivity` reads `intent.data` on startup and in `onNewIntent`.
...
The URI is stored in `_externalUri` for subsequent saves without re-picking.

### writhdeck.ini editing

Gear button in the browser -> `vm.openIniFile()`:
1. Calls `ini-save` first (regenerates the file with current values)
2. Opens the file in the editor

On save, if `entry.name == "writhdeck.ini"`, `reloadConfig()` re-runs `ini-load + keys-init`
in the engine and updates all affected StateFlows (theme colors, heading marker, etc.).
...
### Cross-compilation (`tools/build-tcl-android.sh`)

NDK is auto-detected via `$ANDROID_NDK_ROOT` or `~/Android/Sdk/ndk/<latest version>`.
To override:
```sh
ANDROID_NDK_ROOT=/path/to/ndk ./tools/build-tcl-android.sh arm64-v8a
```

NDK r26+ critical points:
...
- Do not run the build while Android Studio is installing/updating the NDK — partially present `.temp/` files cause exit 127

Output: `tcl-android/<ABI>/install/lib/libtcl8.6.a`
...
| Constraint | Detail |
|---|---|
| No `~` in paths | `tilde-expand` is a no-op on Android — not needed |
| Tcl thread-safety | Single interpreter; all JNI calls must run on `Dispatchers.IO` |
| APK size | `libtcl8.6.a` ~2-4 MB per ABI depending on compile options |
| Tcl `clock` | `clock format` / `clock seconds` work correctly on Android |
| No TUI / Tk | `gui.tcl` and `tui.tcl` excluded — `no_gui = 1` |
| No Tcl event loop | `after`-based procs (`timer-tick`, etc.) cannot be used — use Kotlin coroutines |
| `file normalize` | Works normally — no `~` to handle on Android |
| Permissions | `WRITE_EXTERNAL_STORAGE` not required when staying inside `filesDir` |
| NDK during install | Do not build while Android Studio is installing/updating the NDK |
